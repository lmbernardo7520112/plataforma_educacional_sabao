...
|--------------------|:----------:|:-------:|-------------------------------------|
| **Cópia de Padrão Proprietário ou Assets do Kotobee** | Alta | Jurídico / Direitos | Não copiar nenhum ícone, paleta de cores de balão, código AngularJS ou layout do benchmark. Todo o código HTML/CSS/JS e o design do infográfico do EcoSabon devem ser desenvolvidos originalmente. |
| **Confusão do Hotspot com Simulador IoT ou Cálculo Dinâmico** | Alta | Governança Ética | O hotspot deve exibir **apenas texto conceitual estático explicativo** local. Nenhum input de parâmetros, sliders, fórmulas matemáticas reativas ou cálculos de pH serão adicionados. |
| **Quebra de Impressão (Texto Oculto no PDF)** | Média | Usabilidade Docente | Configurar o `print.css` para ignorar o comportamento toggle. Na impressão, os balões explicativos de todos os hotspots devem ser forçados a renderizar como blocos lineares de texto plano, exibindo as informações na ordem de leitura sem sobreposições. |
...
| **Excesso Decorativo e Poluição Visual** | Baixa | Estética / UX | O visual dos pins de hotspot no infográfico deve ser sutil e minimalista (ex: pequenos círculos com ícones `+` ou `i` que se misturam harmonicamente ao esquema de cores verde/âmbar já consolidado do infográfico). |
| **Layout Quebrado ou Muito Pesado no Mobile** | Baixa | Performance / UX | O infográfico em mobile já empilha verticalmente. As caixas descritivas dos hotspots devem usar posicionamento flexível que ocupe a largura completa da tela do celular, evitando popups flutuantes cortados nas laterais. |
